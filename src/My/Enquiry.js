...
function Enquiry()
{
  ...
  const navigate = useNavigate();
   useEffect(()=>{
      axios.get("http://54.150.175.15:3306/enq").then(res=>setProducts(res.data));
  },[]);

  function onDelete(e){
        
    const id = e.target.id;
    axios.get(`http://54.150.175.15:3306/enuerydelete/${id}`).then(
...
<thead>
<tr><th class="bl5"> Name</th><th class="bl5">Email Address</th><th class="bl5">Phone</th><th class="bl5">Message</th><th class="bl5">Action</th>
             ...
             {product.map((item,index)=>{
                 return(
                     <tr key={index}>
                         ...
                         <td>
                          <Button type="button"class="btn btn-success av"  id={item.id}  onClick={onDelete}>DELETE</Button>
                         </td>
