...
{  
  ...
  useEffect(()=>{
     axios.get("http://54.150.175.15:3306/sub").then(res=>setProducts(res.data));
 },[]);
 ...
 function Piya(e){
  ...
  axios.get(`http://54.150.175.15:3306/pinoa/${id}`).then(
    res => setProducts(res.data));
  
  axios.get(`http://54.150.175.15:3306/noia/${id}`).then(
      res => setProducts(res.data));
      navigate("/Blocked");
...
<div >
<h1 class="h3 mb-0 text-gray-800">sub User List</h1>  
        <Table striped bordered hover >
      <thead>
        <th>Username</th><th>Email</th><th>phone</th><th>sub</th><th class="bl5" >Purchased Date</th><th class="bl5" >Expiry Date</th><th>Active user</th>
      </thead>
         <tbody>
             {product.map((item,index)=>{
                 return(
                     <tr key={index}>
                         ...
                         <td>{item.pdate}</td>
                         <td>{item.edate}</td>
                         <td><Button type="button"  variant="danger" id={item.id} onClick={Piya} >Delete</Button></td>
                     </tr>
                 );
             })}
         </tbody>
