...
function fetchAll(e){
  ...
  parseInt(setAllDocs(e.target.name))
  console.log(allDocs)
  //setEm(e.target.value)
   const docRef = doc(db, 'users' ,allDocs)
  
   onSnapshot(docRef, (doc) =>{
    setLatitude(parseFloat(doc.data().lat, doc.data))
    setEm(doc.data().name, doc.data)
    setLongitude(parseFloat(doc.data().long, doc.data))
   // setLatitude(parseInt(all.lat))
    //setLongitude(parseInt(all.long))
   
  
    })
  
 
   // setAll(snapshot.docs.map(doc => ({id: doc.id}))))
   ...
   //var result = data.docs.map(doc => ({ ...doc.data(), id: doc.id }));
    ...
    //setLongitude(parseInt(e.target.id))
...
{/* 
  ...
  <a class="sidebar-brand d-flex align-items-center justify-content-center" href={"Admin"}>
    ...
    <img class="img-profile rounded-circle" src={"COVER_IMAGE.jpg"} style={{height:50,widows:50}}/>
    </div>
  </a>
...
<!-- Content Wrapper --> */}
         ...
         {/*    <!-- Nav Item - Alerts -->
            ...
            <li class="nav-item dropdown no-arrow">
              <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                <img class="img-profile rounded-circle" src={"COVER_IMAGE.jpg"}/>
                &nbsp; &nbsp;
    ...
    <div >
<h4 class="h3 mb-0 text-gray-800">User List</h4>  
        <Table striped bordered hover >
      <thead>
      <th>Email</th><th>Lat</th>    <th>lon</th><th>Active user</th>
      </thead>
         <tbody>
             {users.map((item,index)=>{
                 return(
                     <tr key={index}>
                         ...
                         <td>{item.lat}</td>
                        <td>{item.long}</td>
                         <td><Button type="button"  variant="danger" value={item.email} name={item.name} onClick={fetchAll} >Map</Button></td>
                     </tr>
                 );
             })}
         </tbody>
