...
{  
  ...
  useEffect(()=>{
     axios.get("http://54.150.175.15:3306/nois").then(res=>setProducts(res.data));
 },[]);
 ...
 function Piya(e){
  ...
  axios.get(`http://54.150.175.15:3306/mino/${id}`).then(
    res => setProducts(res.data));
  
  axios.get(`http://54.150.175.15:3306/kika/${id}`).then(
      res => setProducts(res.data));
      navigate("/Subscribed");
             ...
             {product.map((item,index)=>{
                 return(
                     <tr key={index}>
                         ...
                         <td>{item.pdate}</td>
                         <td>{item.edate}</td>
                         <td><Button type="button"  variant="danger" id={item.id} onClick={Piya} >Activate</Button></td>
                     </tr>
                 );
             })}
         </tbody>
